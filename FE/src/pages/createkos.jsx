...
const CreateKos = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Isi body sesuai gambar: key harus sama persis
      ...
      await axios.post(
        'https://kosanku-tcc-363721261053.us-central1.run.app/kos',
        formData,
        {
          headers: {
  ...
  return (
    <div style={{ maxWidth: 600, margin: '40px auto', padding: 24, background: '#fff', borderRadius: 12, boxShadow: '0 2px 8px #eee', position: 'relative' }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: 12 }}>
          <label>Gambar Kos (bisa lebih dari satu)</label><br />
          <input name="images" type="file" accept="image/*" multiple onChange={handleChange} className="inputfield" />
        </div>
        {error && <div style={{ color: 'red', marginBottom: 12 }}>{error}</div>}
        {success && <div style={{ color: 'green', marginBottom: 12 }}>{success}</div>}
        <button className="button-tombol" type="submit" disabled={loading}>{loading ? 'Menyimpan...' : 'Simpan Kos'}</button>
