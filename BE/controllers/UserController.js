...
async function createUser(req, res) {
  try {
    // Mengambil username, email, password dari request body
    ...
    /*
      ...
      INSERT INTO nama_tabel (username, email, password)
...
async function updateUser(req, res) {
  try {
    // Ambil username, email, dan password dari requerst body
    ...
    /*
      ...
      Keterangan:
      Nilai username, email diambil dari req.body pake spread operator (...) biar gaperlu nulis ulang
...
async function login(req, res) {
  try {
    ...
    // karena kita login pake username & password
    const { username, password } = req.body;

    // Cek apakah username terdaftar di db
    ...
    if (user) {
      ...
      console.log(userPlain); // Cek isi object
      ...
      console.log(safeUserData); // Cek isi object
      ...
      if (isPasswordValid) {
        ...
        // Kalau password salah, masuk ke catch, kasi message "Password atau username salah" (400)
        const error = new Error("Password atau username salah");
        error.statusCode = 400;
        throw error;
      }
    } else {
      // Kalau username salah, masuk ke catch, kasi message "Password atau username salah" (400)
      const error = new Error("Password atau username salah");
      error.statusCode = 400;
      throw error;
    }
